...
export default function NavbarClient({ user, role }: { user: any; role: any }) {
  ...
  // [BARU] State Notifikasi
  const [unreadCount, setUnreadCount] = useState(0);
  const [showNotif, setShowNotif] = useState(false);
  const [notifications, setNotifications] = useState<any[]>([]);

  // Supabase Client untuk Fetch Data di Client Component
  const supabase = createClient();

  // [BARU] Fetch Notifikasi
  useEffect(() => {
    if (!user) return;
    ...
    // Polling setiap 30 detik (Opsional, agar update tanpa refresh)
  ...
  const handleReadNotif = async () => {
    ...
    if (unreadCount > 0) {
      // Tandai semua sudah dibaca di database
      await supabase
        .from("notifications")
        .update({ is_read: true })
        .eq("user_id", user.id);
